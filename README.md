
[![Typing SVG](https://readme-typing-svg.herokuapp.com/?color=A855F7&size=35&center=true&vCenter=true&width=1000&lines=Hello+World,+I'm+Eliana+Alves.;Data+Analytics+%7C+Python+%7C+SQL+%7C+BI)](https://git.io/typing-svg)
<hr>

- 🎓 Formada em Desenvolvimento de Software Multiplataforma.
- 📊 Direcionando minha carreira para a área de Análise de Dados.
- 🐍 Conhecimentos em Python, SQL, PostgreSQL e Power BI.
- 📈 Transformando dados em informações para apoiar decisões.
- 🚀 Desenvolvendo projetos de análise, visualização de dados e Business Intelligence.
- 🌱 Sempre aprendendo e aprimorando minhas habilidades técnicas.

<br>

## 💼 Experiência

### 📊 Trainee em Análise de Dados — Compass UOL
08/2024 - 12/2024

Durante o programa, desenvolvi conhecimentos em Engenharia e Análise de Dados utilizando:
- 📌 Python para tratamento e análise de dados;
- 📌 SQL para consultas e manipulação de bancos;
- 📌 Serviços AWS (S3, Glue, Athena e QuickSight);
- 📌 Construção de pipelines de dados;
- 📌 Visualização e geração de insights.

<br>

## 🛠️ Tecnologias

<div>
  <img height="50" alt="Python" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg">
  <img height="50" alt="PostgreSQL" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg">
  <img height="50" alt="MySQL" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg">
  <img height="50" alt="Docker" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg">
  <img height="50" alt="Git" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg">
</div>

<br>
 
## 📫 Contato

<a href="mailto:elianasilvarodrigues643@gmail.com">
  <img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white">
</a>

<a href="https://www.linkedin.com/in/eliana-alves-098591235/" target="_blank">
  <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white">
</a>

![](https://github.com/BEPb/BEPb/blob/output/github-contribution-grid-snake.svg)
